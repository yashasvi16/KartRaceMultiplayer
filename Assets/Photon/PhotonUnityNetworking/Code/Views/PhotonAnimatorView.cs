...
namespace Photon.Pun
{
    ...
    public class PhotonAnimatorView : MonoBehaviourPun, IPunObservable
    {
        ...
        private void Update()
        {
            if (this.m_Animator[0].applyRootMotion && this.photonView.IsMine == false && PhotonNetwork.IsConnected == true)
            {
                this.m_Animator[0].applyRootMotion = false;
            }
            if (this.m_Animator[1].applyRootMotion && this.photonView.IsMine == false && PhotonNetwork.IsConnected == true)
            {
                this.m_Animator[1].applyRootMotion = false;
            }
            if (this.m_Animator[2].applyRootMotion && this.photonView.IsMine == false && PhotonNetwork.IsConnected == true)
            {
                this.m_Animator[2].applyRootMotion = false;
            }
        ...
        public void CacheDiscreteTriggers()
        {
            for (int i = 0; i < this.m_SynchronizeParameters.Count; ++i)
            {
                ...
                if (parameter.SynchronizeType == SynchronizeType.Discrete && parameter.Type == ParameterType.Trigger && this.m_Animator[0].GetBool(parameter.Name) && this.m_Animator[1].GetBool(parameter.Name) && this.m_Animator[2].GetBool(parameter.Name))
                {
        ...
        private void SerializeDataContinuously()
        {
            ...
            for (int i = 0; i < this.m_SynchronizeLayers.Count; ++i)
            {
                ...
                {
                    this.m_StreamQueue.SendNext(this.m_Animator[0].GetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex));
                    this.m_StreamQueue.SendNext(this.m_Animator[1].GetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex));
                    this.m_StreamQueue.SendNext(this.m_Animator[2].GetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex));
                }
            }
                ...
                if (parameter.SynchronizeType == SynchronizeType.Continuous)
                {
                    switch (parameter.Type)
                    {
                        case ParameterType.Bool:
                            {
                                this.m_StreamQueue.SendNext(this.m_Animator[0].GetBool(parameter.Name));
                                this.m_StreamQueue.SendNext(this.m_Animator[1].GetBool(parameter.Name));
                                this.m_StreamQueue.SendNext(this.m_Animator[2].GetBool(parameter.Name));
                                break;
                            }
                        case ParameterType.Float:
                            {
                                this.m_StreamQueue.SendNext(this.m_Animator[0].GetFloat(parameter.Name));
                                this.m_StreamQueue.SendNext(this.m_Animator[1].GetFloat(parameter.Name));
                                this.m_StreamQueue.SendNext(this.m_Animator[2].GetFloat(parameter.Name));
                                break;
                            }
                        case ParameterType.Int:
                            {
                                this.m_StreamQueue.SendNext(this.m_Animator[0].GetInteger(parameter.Name));
                                this.m_StreamQueue.SendNext(this.m_Animator[1].GetInteger(parameter.Name));
                                this.m_StreamQueue.SendNext(this.m_Animator[2].GetInteger(parameter.Name));
                                break;
                            }
        ...
        private void DeserializeDataContinuously()
        {
            ...
            for (int i = 0; i < this.m_SynchronizeLayers.Count; ++i)
            {
                if (this.m_SynchronizeLayers[i].SynchronizeType == SynchronizeType.Continuous)
                {
                    this.m_Animator[0].SetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex, (float) this.m_StreamQueue.ReceiveNext());
                    this.m_Animator[1].SetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex, (float)this.m_StreamQueue.ReceiveNext());
                    this.m_Animator[2].SetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex, (float)this.m_StreamQueue.ReceiveNext());
                }
            }
            ...
            for (int i = 0; i < this.m_SynchronizeParameters.Count; ++i)
            {
                ...
                if (parameter.SynchronizeType == SynchronizeType.Continuous)
                {
                    ...
                    {
                        case ParameterType.Bool:
                            this.m_Animator[0].SetBool(parameter.Name, (bool) this.m_StreamQueue.ReceiveNext());
                            this.m_Animator[1].SetBool(parameter.Name, (bool)this.m_StreamQueue.ReceiveNext());
                            this.m_Animator[2].SetBool(parameter.Name, (bool)this.m_StreamQueue.ReceiveNext());
                            break;
                        case ParameterType.Float:
                            this.m_Animator[0].SetFloat(parameter.Name, (float) this.m_StreamQueue.ReceiveNext());
                            this.m_Animator[1].SetFloat(parameter.Name, (float)this.m_StreamQueue.ReceiveNext());
                            this.m_Animator[2].SetFloat(parameter.Name, (float)this.m_StreamQueue.ReceiveNext());
                            break;
                        case ParameterType.Int:
                            this.m_Animator[0].SetInteger(parameter.Name, (int) this.m_StreamQueue.ReceiveNext());
                            this.m_Animator[1].SetInteger(parameter.Name, (int)this.m_StreamQueue.ReceiveNext());
                            this.m_Animator[2].SetInteger(parameter.Name, (int)this.m_StreamQueue.ReceiveNext());
                            break;
                        case ParameterType.Trigger:
                            this.m_Animator[0].SetBool(parameter.Name, (bool) this.m_StreamQueue.ReceiveNext());
                            this.m_Animator[1].SetBool(parameter.Name, (bool)this.m_StreamQueue.ReceiveNext());
        ...
        private void SerializeDataDiscretly(PhotonStream stream)
        {
            for (int i = 0; i < this.m_SynchronizeLayers.Count; ++i)
            {
                ...
                {
                    stream.SendNext(this.m_Animator[0].GetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex));
                    stream.SendNext(this.m_Animator[1].GetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex));
                    stream.SendNext(this.m_Animator[2].GetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex));
                }
            }
            ...
            for (int i = 0; i < this.m_SynchronizeParameters.Count; ++i)
            {
                ...
                if (parameter.SynchronizeType == SynchronizeType.Discrete)
                {
                    ...
                    {
                        case ParameterType.Bool:
                            stream.SendNext(this.m_Animator[0].GetBool(parameter.Name));
                            stream.SendNext(this.m_Animator[1].GetBool(parameter.Name));
                            stream.SendNext(this.m_Animator[2].GetBool(parameter.Name));
                            break;
                        case ParameterType.Float:
                            stream.SendNext(this.m_Animator[0].GetFloat(parameter.Name));
                            stream.SendNext(this.m_Animator[1].GetFloat(parameter.Name));
                            stream.SendNext(this.m_Animator[2].GetFloat(parameter.Name));
                            break;
                        case ParameterType.Int:
                            stream.SendNext(this.m_Animator[0].GetInteger(parameter.Name));
                            stream.SendNext(this.m_Animator[1].GetInteger(parameter.Name));
                            stream.SendNext(this.m_Animator[2].GetInteger(parameter.Name));
                            break;
                        case ParameterType.Trigger:
                            if (!TriggerUsageWarningDone)
                            {
        ...
        private void DeserializeDataDiscretly(PhotonStream stream)
        {
            for (int i = 0; i < this.m_SynchronizeLayers.Count; ++i)
            {
                if (this.m_SynchronizeLayers[i].SynchronizeType == SynchronizeType.Discrete)
                {
                    this.m_Animator[0].SetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex, (float) stream.ReceiveNext());
                    this.m_Animator[1].SetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex, (float)stream.ReceiveNext());
                    this.m_Animator[2].SetLayerWeight(this.m_SynchronizeLayers[i].LayerIndex, (float)stream.ReceiveNext());
                }
            }
                ...
                if (parameter.SynchronizeType == SynchronizeType.Discrete)
                {
                    switch (parameter.Type)
                    {
                        case ParameterType.Bool:
                            ...
                            this.m_Animator[0].SetBool(parameter.Name, (bool) stream.ReceiveNext());
                            this.m_Animator[1].SetBool(parameter.Name, (bool)stream.ReceiveNext());
                            this.m_Animator[2].SetBool(parameter.Name, (bool)stream.ReceiveNext());
                            break;
                        case ParameterType.Float:
                            ...
                            this.m_Animator[0].SetFloat(parameter.Name, (float) stream.ReceiveNext());
                            this.m_Animator[1].SetFloat(parameter.Name, (float)stream.ReceiveNext());
                            this.m_Animator[2].SetFloat(parameter.Name, (float)stream.ReceiveNext());
                            break;
                        case ParameterType.Int:
                            ...
                            this.m_Animator[0].SetInteger(parameter.Name, (int) stream.ReceiveNext());
                            this.m_Animator[1].SetInteger(parameter.Name, (int)stream.ReceiveNext());
                            this.m_Animator[2].SetInteger(parameter.Name, (int)stream.ReceiveNext());
                            break;
                        case ParameterType.Trigger:
                            if (stream.PeekNext() is bool == false)
                            {
                            ...
                            {
                                this.m_Animator[0].SetTrigger(parameter.Name);
                                this.m_Animator[1].SetTrigger(parameter.Name);
                                this.m_Animator[2].SetTrigger(parameter.Name);
                            }
                            break;
                    }
                }
